...
const backgrounds = {
  ...
  WarpFBM: WarpFBMBackground,
  VectorField: VectorFieldBackground,
} as const  // ✅ make keys readonly

type BackgroundKey = keyof typeof backgrounds // "Liquid" | "CosWave" | "WarpFBM" | "VectorField"

export default function BackgroundManager() {
  const [active, setActive] = useState<BackgroundKey>('VectorField')

  const ActiveBG = backgrounds[active] // ✅ now TS knows this is valid
  ...
  return (
    <div className="h-screen w-full bg-black relative overflow-hidden select-none">
      ...
      <div className="absolute top-6 left-1/2 -translate-x-1/2 flex gap-4 z-50">
        {Object.keys(backgrounds).map((name) => {
          const key = name as BackgroundKey // ✅ cast for TS
          return (
            <button
              key={key}
              ...
              className={`
                ...
                shadow-lg
                ${active === key ? 'bg-red-500/70 text-white scale-110 shadow-2xl' : 'hover:bg-white/20 hover:scale-105'}
              `}
            >
              {key}
            </button>
